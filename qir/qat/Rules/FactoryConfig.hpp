...
public:
  void setup(ConfigurationManager &config)
  {
    ...
    config.addParameter(disable_reference_counting_, true, "disable-reference-counting",
                        "Disables reference counting by instruction removal.");
    config.addParameter(disable_alias_counting_, true, "disable-alias-counting",
                        "Disables alias counting by instruction removal.");
    config.addParameter(disable_string_support_, true, "disable-string-support",
  ...
  bool isDisabled() const
  {
    return (disable_reference_counting_ == false && disable_alias_counting_ == false &&
            disable_string_support_ == false && optimise_result_one_ == false &&
            optimise_result_zero_ == false && use_static_qubit_array_allocation_ == false &&
