...
RuleFactory::RuleFactory(RuleSet &rule_set, AllocationManagerPtr qubit_alloc_manager,
                         AllocationManagerPtr result_alloc_manager, ILoggerPtr logger)
  : rule_set_{rule_set}
...
void RuleFactory::useStaticQubitAllocation()
{
  ...
  addRule(
      {call("__quantum__rt__qubit_release", "name"_cap = _),
       [qubit_alloc_manager, deleter, logger](Builder &builder, Value *val, Captures &cap,
                                              Replacements &rep) {
         // Getting the name
...
{
  auto replace_constant_result = [](Builder &builder, Value *val, Captures &cap,
                                    Replacements &replacements) {
  ...
  */
  ...
  addRule({call("__quantum__rt__result_equal", "1"_cap = get_zero, "2"_cap = get_zero),
           replace_constant_result});
  addRule({call("__quantum__rt__result_equal", "1"_cap = get_zero, "2"_cap = get_one),
           replace_constant_result});
  addRule({call("__quantum__rt__result_equal", "1"_cap = get_one, "2"_cap = get_zero),
           replace_constant_result});
  addRule({call("__quantum__rt__result_equal", "1"_cap = get_one, "2"_cap = get_one),
           replace_constant_result});
}
