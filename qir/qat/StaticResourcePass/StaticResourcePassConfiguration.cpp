...
void StaticResourcePassConfiguration::setup(ConfigurationManager &config)
{
  ...
  config.addParameter(annotate_max_qubit_index_, "annotate-max-qubit-index",
                      "Annotate the maximum qubit index used");

  config.addParameter(annotate_max_result_index_, "annotate-max-result-index",
                      "Annotate the maximum result index used");

  config.addParameter(annotate_max_result_index_, "annotate-max-result-index",
                      "Annotate the maximum result index used");

  config.addParameter(reindex_qubits_, "reindex-qubits",
                      "Re-indexes statically allocated qubits with sequential ids starting from 0");

  config.addParameter(replace_qubit_on_reset_, "replace-qubit-on-reset",
                      "Replaces a qubit with new qubit if the qubit is reset");
}
