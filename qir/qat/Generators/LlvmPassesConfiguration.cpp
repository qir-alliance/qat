...
{
  config.setSectionName("LLVM optimizations",
                        "Enables specific LLVM optimizations before and after transformation.");
  ...
  config.addParameter(unroll_opt_level_, "unroll-opt-level",
                      "Sets the optimization level for loop unrolling.");
  config.addParameter(
  ...
  config.addParameter(unroll_forget_scev_, "forget-scev",
  ...
  config.addParameter(eliminate_constants_, "eliminate-constants",
                      "Uses LLVM pass to eliminate constants.");
  config.addParameter(eliminate_dead_code_, "eliminate-dead-code",
                      "Uses LLVMs aggressive dead code elimination.");
  config.addParameter(eliminate_memory_, "eliminate-memory",
                      "Maps memory into registers where feasible.");
...
bool LlvmPassesConfiguration::isDisabled() const
{
  return always_inline_ == false && inline_parameter_ == 0 && unroll_loops_ == false &&
         unroll_allow_partial_ == false && unroll_allow_peeling_ == false &&
         unroll_allow_runtime_ == false && unroll_allow_upper_bound_ == false &&
         unroll_allow_profile_based_peeling_ == false && unroll_full_unroll_count_ == 0 &&
         unroll_opt_level_ == 0 && unroll_only_when_forced_ == false &&
         unroll_forget_scev_ == false && eliminate_constants_ == false &&
         eliminate_dead_code_ == false && eliminate_memory_ == false &&
         use_llvm_opt_pipeline_ == false && opt_pipeline_config_ == "";
}
