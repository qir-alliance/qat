...
void TransformationRulesPassConfiguration::setup(ConfigurationManager &config)
{
  config.setSectionName("Pass configuration",
                        "Configuration of the pass and its corresponding optimizations.");
  ...
  config.addExperimentalParameter(transform_execution_path_only_, true, true,
  ...
  config.addParameter(reuse_qubits_, false, "reuse-qubits",
                      "Use to define whether or not to reuse qubits.");
  config.addParameter(reuse_results_, false, "reuse-results",
    ...
    TransformationRulesPassConfiguration const &ref) const
{
  ...
  return (delete_dead_code_ == ref.delete_dead_code_ && clone_functions_ == ref.clone_functions_ &&
          ...
          reuse_qubits_ == ref.reuse_qubits_);
}
