                                            ...
                                            InstructionModifier const &modifier)
{
  ...
  if (depth_ >= config_.maxRecursion())
  {
    ...
    logger_->error("Function inlining exceeded maximum recursion depth of " +
                   std::to_string(config_.maxRecursion()));
  ...
  for (auto it = replacements_.rbegin(); it != replacements_.rend(); ++it)
  {
    ...
    if (it->second != nullptr)
    {
      ...
      if (instr2 == nullptr)
      {
        requireLogger();
        logger_->setLocationFromValue(instr1);
        logger_->internalError(
            "Attempt to replace with non-instruction value. This is not allowed.");
        continue;
      }
    ...
    for (auto &block : function)
    {
      ...
      for (auto &instr : block)
      {
        ...
        if (rule->match(&instr, captures))
        {
          ...
          auto block1 =
              phi->getIncomingBlock(0);  // TODO(issue-21): Make sure that block1 matches val1
...
void TransformationRulesPass::runApplyRules(llvm::Module &module, llvm::ModuleAnalysisManager &)
{
  ...
  for (auto &function : module)
  {
    ...
    if (function.hasFnAttribute(config_.entryPointAttr()))
    {
      runOnFunction(function, [this, &already_visited](llvm::Value *value,
          ...
          for (auto &op : instr.operands())
          {
            ...
            if (a || b)
            {
              ...
              if (user && user->getNumOperands() == 1)
              {
                ...
                if (cst)
                {
                  auto n = cst->getValue().getZExtValue();
...
void TransformationRulesPass::requireLogger()
{
  ...
  {
    throw std::runtime_error(
        "TransformationRulesPass does not have a logger, but need it to emit messages.");
  }
}
