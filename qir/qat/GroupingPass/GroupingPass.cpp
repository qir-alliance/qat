...
namespace quantum
{
    ...
    void GroupingPass::expandBlockQuantumClassical(llvm::Module& module, llvm::BasicBlock* tail_classical)
    {
        ...
        auto quantum_block = llvm::BasicBlock::Create(context, "quantum", tail_classical->getParent(), tail_classical);
        ...
        for (auto& instr : *tail_classical)
        {
            ...
            if (call_instr != nullptr)
            {
                ...
                if (f != nullptr)
                {
                    ...
                    bool is_readout =
                        (name.size() >= GroupingAnalysisPass::READ_INSTR_START.size() &&
    ...
    void GroupingPass::expandBlockQuantumMeasureClassical(llvm::Module& module, llvm::BasicBlock* tail_classical)
    {
        ...
        auto readout_block = llvm::BasicBlock::Create(context, "readout", tail_classical->getParent(), tail_classical);

        auto quantum_block = llvm::BasicBlock::Create(context, "quantum", tail_classical->getParent(), readout_block);
                ...
                if (f != nullptr)
                {
                    ...
                    if (is_quantum)
                    {
                        ...
                        if (new_quantum_measure_cycle)
                        {
                            ...
                            quantum_block = llvm::BasicBlock::Create(
                                context, "quantum", tail_classical->getParent(), tail_classical);

                            readout_builder.CreateBr(quantum_block);
                            readout_block = llvm::BasicBlock::Create(
                                context, "readout", tail_classical->getParent(), tail_classical);
