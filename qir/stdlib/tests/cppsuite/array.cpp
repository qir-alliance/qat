...
template <typename T> void intTest(int64_t count)
{
...
template <typename T> void testLazyCopy(int64_t count)
{
    ArrayTest<T> arr(count);
    assert(arr.size() == count);
    assert(arr.elementSize() == sizeof(T));
...
template <typename T> void testRefCountRecurse(ArrayTest<T>& orig, int64_t depth)
{
    if (depth > 20)
    {
        return;
...
template <typename T> void testRefCount(int64_t count)
{
    ArrayTest<T> arr(count);
    for (int64_t i = 0; i < arr.size(); ++i)
    {
