...
def require_token(
    token: str, filename: str, contents: str, cursor: int, fix_issues: bool
) -> int:
    """
    ...
    if not contents[cursor:].startswith(token):
        logger.error(
            "{}: File must have {} at position {}".format(filename, token, cursor)
        )
        failed = True
    return cursor + len(token), failed


def require_pragma_once(
    filename: str, contents: str, cursor: int, fix_issues: bool
) -> int:
    """
        ...
        if fix_issues:
            ...
            sys.stdout.writelines(
                difflib.unified_diff(
                    ...
                    formatted,
                    fromfile=filename,
                    tofile="formatted_{}".format(filename),
                )
            )
            sys.stdout.writelines("\n\n")
        return cursor, True
...
def enforce_cpp_formatting(
    ...
    assumed_filename = filename.replace(PROJECT_ROOT, "")
    if assumed_filename.startswith("/"):
        assumed_filename = assumed_filename[1:]

    p = subprocess.Popen(
        [CLANG_FORMAT_EXE, "--assume-filename={}".format(assumed_filename), "-style=file"], 
        stdout=subprocess.PIPE,
        stdin=subprocess.PIPE,
        cwd=PROJECT_ROOT,
    )
...
def enforce_quantum_symbols(
    ...
    for match in QUANTUM_SYMBOL_PATTERN.finditer(contents):
        ...
        specialization = ""
        try:
            instr, specialization = instr.rsplit("__", 1)
        except:  # noqa: E722
            pass
        ...
        if type == "qis":
            if instr not in VALID_INSTRUCTIONS:
                ...
                fail = True
            elif specialization not in ["body", "ctl", "adj", "adjctl"]:
                logger.error(
                    "Missing specialization for quantum symbol: {} in {}".format(
                        full, filename
                    )
                )
                fail = True

        if (end != ":") and (start == '"' or end == '"') and start != end:
            logger.error(
                "Invalid string format for: '{}{}{}' in {}".format(
                    start, full, end, filename
                )
            )
        elif (end != ":") and (start == '"' or end == '"') and start != end:
            logger.error(
                'Invalid string format for: "{}{}{}" in {}'.format(
                    start, full, end, filename
                )
...
def enforce_prettier_formatting(
    basedir: str, extensions: str, fix_issues: bool, options: Optional[List[str]] = None
) -> bool:
    ...
    if "," in extensions:
        cmd = (
            ["prettier"]
            + options
            + [mode, '"{}'.format(basedir) + "(**/)?*.{" + extensions + '}"']
        )
    else:
        cmd = (
            ["prettier"]
    ...
    try:
        p = subprocess.Popen(" ".join(cmd), cwd=PROJECT_ROOT, shell=True)
    except:  # noqa: E722
...
def enforce_markdown_linting(basedir: str, fix_issues: bool) -> bool:
...
def enforce_prettier_yaml_formatting(basedir: str, fix_issues: bool) -> bool:
    return enforce_prettier_formatting(basedir, "yaml,yml", fix_issues)


def enforce_prettier_json_formatting(basedir: str, fix_issues: bool) -> bool:
    return enforce_prettier_formatting(basedir, "json", fix_issues)


def enforce_prettier_docs_formatting(basedir: str, fix_issues: bool) -> bool:
    return enforce_prettier_formatting(
        basedir, "md", fix_issues, options=["--prose-wrap", "always"]
    ) and enforce_prettier_formatting(
        basedir, "css,scss,js,html", fix_issues
    )  # and \
...
SOURCE_PIPELINES = [
    ...
    },
    {
        ...
        "pipelines": {
            ...
            "c": [enforce_cpp_license, enforce_cpp_formatting, enforce_quantum_symbols],
        },
    },
    {
        ...
        "pipelines": {
            "py": [
                enforce_py_license
            ],  # , enforce_py_formatting, enforce_quantum_symbols],
        },
    },
    {
        "name": "Documentation",
    ...
    for language in SOURCE_PIPELINES:
        ...
        for root, dirs, files in os.walk(basedir):
            ...
            for filename in files:
                ...
                if ext in pipelines:
                    f = execute_pipeline(
                        pipelines[ext], path.join(root, filename), fix_issues
                    )
                    if f:
                        logger.error(
                            "Failed checks for {}.{} ".format(language["name"], ext)
                        )
                    failed = failed or f
