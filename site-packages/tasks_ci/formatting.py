...
def enforce_py_formatting(filename: str, contents: str,
    ...
    try:
        ...
        logger.info("Error: autopep 8 not available. Please install it")
        return cursor, True
...
def enforce_prettier_formatting(
        basedir: str,  extensions: str, fix_issues: bool) -> bool:
    ...
    if "," in extensions:
        cmd = ["prettier", mode, '"{}'.format(basedir) + '(**/)?(.)*.{'+extensions+'}"']
    else:
        cmd = ["prettier", mode, '"{}'.format(basedir) + '(**/)?(.)*.'+extensions+'"']
    ...
    try:
        ...
        logger.info("Error: prettier not available. Please install it")
        return False
    ...
    if p.returncode != 0:
        logger.info("Error: Files in documentation does not follow style guide")
        return False
...
def enforce_prettier_docs_formatting(
        basedir: str,  fix_issues: bool) -> bool:
    return enforce_prettier_formatting(basedir, "md,css,scss,js,html", fix_issues)
...
def main(fix_issues: bool = False) -> None:
    ...
    for language in SOURCE_PIPELINES:
        ...
        if "bulk-apply" in language:
            ...
            failed = (not f(basedir, fix_issues)) or failed
            continue
