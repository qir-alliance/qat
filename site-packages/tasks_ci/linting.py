...
from .builder import build_project, configure_cmake
...
def clang_tidy_diagnose() -> None:
    ...
    config = subprocess.check_output(
        [toolchain.discover_tidy(), "-dump-config"], cwd=PROJECT_ROOT
    ).decode()

    # Getting the list of checks
    check_list = subprocess.check_output(
        [toolchain.discover_tidy(), "-list-checks"], cwd=PROJECT_ROOT
    ).decode()
...
def main_cpp(fix_issues: bool) -> bool:
    ...
    # TODO(issue-24): Ensure that it is only those which were changed that are
    # analysed
    files_to_analyse = []
