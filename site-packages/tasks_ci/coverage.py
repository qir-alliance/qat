...
def find_test_executables(build_dir):
    ...
    output = subprocess.check_output(
        [toolchain.discover_ctest(), "-N", "--debug"], cwd=build_dir
    ).decode("utf-8")
...
def main(build_dir: str, output_dir: Union[str, None] = None, output_format="html"):
    ...
    for exe in test_executables:
        ...
        try:
            cmd2 = [
                toolchain.discover_llvm_cov(),
                "show",
                '-ignore-filename-regex=".*(vendors|llvm|Tests).*"',
                "-Xdemangler",
                "c++filt",
                binary,
                "-instr-profile=" + index_file,
    ...
    cmd = (
        ...
        + all_binaries
        + [
            "-instr-profile=" + summary_index_file,
            "-format={}".format(output_format),
            "-o",
            current_output_dir,
        ]
