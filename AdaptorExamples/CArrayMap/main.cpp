...
class CArrayMapConfig
{
  public:
    ...
    void setup(ConfigurationManager& config)
    {
        ...
        config.addParameter(replace_allocators_, "replace-allocators", "Replace allocators with C++ allocators.");

        config.addParameter(
            remove_array_copies_, "remove-array-copies",
            "Remove all array copies and replace them with the original array.");

        config.addParameter(
            repalce_access_operators_, "replace-access-operators", "Assumes that allocators use continuous memory.");
    }
...
void activateAllocatorReplacement(RuleSet& ruleset)
{
    ...
    auto replacer = [](ReplacementRule::Builder& builder, ReplacementRule::Value* val,
                       ReplacementRule::Captures& captures, ReplacementRule::Replacements& replacements)
    {
    ...
    ruleset.addRule({call("__quantum__rt__array_create_1d", "size"_cap = _, "count"_cap = _), replacer});
}
...
void replaceAccess(RuleSet& ruleset)
{
    ...
    ruleset.addRule(
        {call(
             ...
             "array"_cap = call("__quantum__rt__array_create_1d", "size"_cap = _, "count"_cap = _), "index"_cap = _),
         replacer});

    ruleset.addRule({callByNameOnly("__quantum__rt__array_update_reference_count"), deleteInstruction()});
...
{
    using namespace microsoft::quantum::notation;
    auto replacer = [](ReplacementRule::Builder&, ReplacementRule::Value* val, ReplacementRule::Captures& captures,
